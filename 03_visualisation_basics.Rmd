...
We can use different geoms on a plot. In one of the examples above, the first one is a scatter plot, which uses a point geom, the next one is a smooth geom, which adds a smooth line to the data to see trends.  
I can combine both together into a single plot with ggplot2;  
...
From the above, I can see that, we can combine different geometric properties, as well as aestistics for these geoms together in a single plot.  From the plot